...
exports.changePassword = async (req, res) => {
  try {
    ...
    // User.replaceOne({ email: email }, { password: hashedPassword });
    // new true return the updated details
    const newUserInfo = await db.collection.findOneAndReplace( { email: email }, { password: hashedPassword }, { new:true })


    // Send mail that password  is changed
    const subject='Password change successful'
    const mail ="<h2>Password Change Successful</h2><p>The password has been changed for your study notion application</p>";
    await SendMail(user.email, subject, mail);

  } catch (err) {
    return res.status(400).json({
      success: false,
      message: "error ucured while changing password ",
    });
  }
};
